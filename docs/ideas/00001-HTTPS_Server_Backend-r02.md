---
title: "Idea 00001 r02: HTTPS Server Backend"
aliases:
  - "Idea 00001"
tags:
  - idea
...
created: 2026-09-18
updated: 2026-09-18
analysed_at: "2026-09-18T15:27:42Z"
agent: Claude Code
model: "anthropic/claude-fable-5-1"
triggered_by: user
previous_revision: "[[00001-HTTPS_Server_Backend-r01]]"
...
# Idea 00001 r02: HTTPS Server Backend
...
>
> **r02** incorporates the review of r01 (findings R1 to R5, §16): it
> removes an unsourced "2 GiB" item size, corrects the `Store` method count,
> adds upload replay semantics as a new Medium finding and to the spike, and
> makes the control database fail closed. The recommendation, the two Majors,
> and the three blocking questions are unchanged.
...
- A large upload is cut off shortly before its end: nothing may appear, and
  the staging space must come back (MED-01).
- The connection drops after the server committed an upload but before the
  ...
  error and not a second item (MED-07).
- The control database is locked or damaged while requests arrive (MED-04).
...
#### IDEA-00001-R02-MAJ-01: Changing encryption needs a new protocol, not a new transport
...
#### IDEA-00001-R02-MAJ-02: A typed item schema and a zero-knowledge server pull in opposite directions
...
#### IDEA-00001-R02-MED-01: GraphQL is the wrong tool for the bytes, and a new attack surface for the rest

> [!note] Medium
> - **Confidence:** High
> - **Category:** Architecture / Security
> - **Evidence:** Item size is unbounded: `ItemMeta.size` is a `u64` and the client sets no limit. The client streams content without buffering the whole of it (`Store::put` doc). GraphQL responses are JSON; multipart upload extensions exist but there is no streaming download, no `Range`, no resume.
> - **Failure scenario:** Content is base64 in JSON and the server buffers whole items; or an unauthenticated deeply nested query burns CPU.
> - **Impact:** Memory exhaustion; denial of service.
> - **Mitigation or test:** Hybrid. GraphQL carries control and metadata; two plain endpoints carry bytes: `PUT /v1/uploads/{uploadId}` and `GET /v1/items/{id}/content` with `Range`. Authenticate before parsing the query; cap depth, complexity, and body size; disable introspection unless configured. A server-side `max_item_bytes` is a limit the `ssh` and `local` backends do not have, so it is a parity gap by design: it needs its own error code (`ITEM_TOO_LARGE`), a value the client can read before it uploads (`ServerInfo.maxItemBytes`), and an "unlimited" setting; its default is an output of the spike. Since the API is small, record honestly that REST would also do (§11); GraphQL earns its place through the single-round-trip `list`, a typed SDL contract shared by two repositories, and later subscriptions.
> - **References:** Client `store/mod.rs`, `model.rs`; review finding R1.

#### IDEA-00001-R02-MED-02: Self-hosted TLS needs a trust story as strong as the pinned SSH host key
...
#### IDEA-00001-R02-MED-03: "Proprietary" and "published on Docker Hub" conflict until terms exist
...
#### IDEA-00001-R02-MED-04: The CLI and the daemon share state, and revocation must be immediate
...
#### IDEA-00001-R02-MED-05: Who mints the id, and deduplication must become atomic
...
#### IDEA-00001-R02-MED-06: Expiring keys fail silently on unattended devices
...
#### IDEA-00001-R02-MED-07: Upload replay semantics are unspecified

> [!note] Medium
> - **Confidence:** High
> - **Category:** Data / Operations
> - **Evidence:** Upload is three requests (`beginUpload`, `PUT`, `commitUpload`), and r01 defined none of them under retry. `serve` retries failed uploads with back-off (client `docs/architecture.md`, "serve"), so replays are the normal case on a poor link, not an exotic one.
...
#### IDEA-00001-R02-LOW-02: Two repositories, one contract
...
#### IDEA-00001-R02-INFO-01: The server may reuse `passalong-core`
...
repeat, with the upload id as the idempotency key (MED-07). Files on disk are
...
| Control database locked, unreadable, or corrupt | Operations | Low | High | Fail closed with 503; `/readyz` fails, `/healthz` stays up; never serve remembered authentication (MED-04) |
| Lost responses during upload | Data | High on poor links | Medium | Replay semantics keyed on the upload id (MED-07) |
| Server item-size limit surprises users of a backend that had none | Adoption | Medium | Low | Distinct error, readable limit, "unlimited" setting; default decided in the spike (MED-01) |
...
- **Method:** (1) Write `docs/api/schema.graphql` and the two content
  endpoints in full, including the replay semantics of every mutation and
  of `PUT` (MED-07), and a decided default for `maxItemBytes` (MED-01). (2) In this repository, implement the workspace state
...
- **Success threshold:** Every `Store` method and `encrypt` mode mapped;
  every mutation's replay behaviour stated; the model test passes; the trait sketch touches no sealing code, only
  orchestration.
...
- **Evidence to capture:** The schema, the model test, the trait sketch, a
...
- [ ] The default for `limits.max_item_bytes`, and whether "unlimited" is
      allowed: the other backends have no limit (MED-01). Decided in the
      spike.
...
Review of r01, received 2026-09-18.

| Feedback or prior finding | Disposition | Change in this revision | Rationale |
|---|---|---|---|
| R1: "Items reach 2 GiB" is unsourced, and is missing from §9 | accepted | MED-01 evidence and the §4 edge case reworded; two rows added to §9; the `maxItemBytes` default added to the spike, §13, and §15 | Verified: the client's sizes are `u64` with no cap. The figure was this repository's own proposed config default, cited as if it were a fact about the client. Unbounded size strengthens MED-01 |
| R2: "`Store` (13 methods)" is loose | accepted | §2 now says 14, plus the deprecated `newest_id`; a row added to §9 | Verified: 15 in the trait. 13 was the row count of the mapping table, which merges `list` with `list_after` and `key_id` with `content_key` |
| R3: `commitUpload` idempotency is unspecified | accepted | New finding MED-07; §4, §12, §13, and §14 extended | Extended beyond the review: a lost `beginUpload` response leaks a quota reservation, so `beginUpload` must be idempotent too |
| R4a: the control database's failure mode is unstated; fail closed | accepted | MED-04 evidence, failure scenario, and mitigation; §13 row | Added `/readyz` failing while `/healthz` stays up, so orchestrators do not restart-loop on a disk fault |
| R4b: `docker compose exec` defaults to root, so documents must exec as uid 10001 | partially-accepted | MED-04 states where the owner rule actually bites; §9 row | `exec` uses the container's configured user, and the `Dockerfile` sets `USER passalong-server`, so the project's image already execs as uid 10001. The rule guards against `user: root`, `--user 0`, and `sudo` on native installs. No `--user` flag is added to the examples; the documents explain the refusal instead. Docker's behaviour was not tested in this session |
| R5: nothing is committed | accepted | Decision log | r01 was in the working tree only (`git add -N` records paths, not content). The user committed it as `f3e7961` before this revision |

Prior formal findings, reassessed:

| r01 finding | Classification | r02 finding |
|---|---|---|
| IDEA-00001-R01-MAJ-01 | still-open | IDEA-00001-R02-MAJ-01, unchanged |
| IDEA-00001-R01-MAJ-02 | still-open | IDEA-00001-R02-MAJ-02, unchanged |
| IDEA-00001-R01-MED-01 | still-open | IDEA-00001-R02-MED-01, evidence corrected (R1), item-size parity added |
| IDEA-00001-R01-MED-02 | still-open | IDEA-00001-R02-MED-02, unchanged |
| IDEA-00001-R01-MED-03 | still-open | IDEA-00001-R02-MED-03, unchanged |
| IDEA-00001-R01-MED-04 | still-open | IDEA-00001-R02-MED-04, extended (R4) |
| IDEA-00001-R01-MED-05 | still-open | IDEA-00001-R02-MED-05, unchanged |
| IDEA-00001-R01-MED-06 | still-open | IDEA-00001-R02-MED-06, unchanged |
| IDEA-00001-R01-LOW-01 | still-open | IDEA-00001-R02-LOW-01, unchanged |
| IDEA-00001-R01-LOW-02 | still-open | IDEA-00001-R02-LOW-02, unchanged |
| IDEA-00001-R01-INFO-01 | still-open | IDEA-00001-R02-INFO-01, unchanged |
| none | new | IDEA-00001-R02-MED-07 (R3) |
...
1. Answer the three blocking questions of §15, which the review of r01 did
   not address.
2. Bring the living drafts in line with this revision, under a plan or by
   the user's instruction, since an idea report changes nothing outside
   `docs/ideas/`: replay semantics and the 503 in `docs/api/README.md`;
   fail-closed behaviour in `docs/architecture.md`; the item-size note in
   `docs/configuration.md` and `config.sample.toml`; the refusal to run as
   another user in `docs/usage.md`.
3. Run the experiment in §14 on a `feature/` branch.
4. On success, accept the idea (a new revision, by the user's decision) and
...
| r02 | revised | feedback | r01 | Review findings R1 to R5: unsourced item size removed, method count corrected, upload replay semantics added as MED-07, control database fails closed |
...
dependency has been audited against `deny.toml`. The review of r01 found
one unsourced claim; every claim r02 adds is in §9 with its evidence.
